...
We extract a 35-dimensional feature vector $\mathbf{x}$:
\begin{itemize}
...
\FloatBarrier
% --- METHOD E (NEW - From Heng) ---
\subsection{Method E: Bidirectional GraphSAGE-LSTM with Data Augmentation and Weighted Cross-Entropy} 
\label{sec:method_E} 
...
\subsubsection{Data Augmentation Strategy} 
We implement a comprehensive Trojan insertion methodology to expand the limited training dataset. Our augmentation process parses Trojan definitions from text files to extract gate-level components, which are then strategically inserted into clean benchmark circuits with unique naming (`HT\_'' prefix) to avoid conflicts. The augmentation randomly selects replacement ratios (typically 20\% for Trojan gates and 20\% for naive replacements) to generate diverse circuit variants, creating approximately 8-12 augmented circuits per original design, significantly expanding the training corpus. 

\subsubsection{Feature Extraction} 
We extract a comprehensive 21-dimensional feature vector capturing both structural and topological characteristics including graph centrality metrics (degree centrality, betweenness centrality, clustering coefficient), gate connectivity patterns (number of inputs/outputs), node type encoding (is\_input, is\_output, is\_gate), gate family one-hot encoding for 9 gate types (AND, OR, NAND, NOR, NOT, BUF, XOR, XNOR, DFF), and distance-based features (PI: shortest path to nearest input port, PO: shortest path to nearest output port). Selected features undergo normalization to ensure stable training convergence. 

\subsubsection{Model Architecture} 
The model employs a 3-layer Bidirectional GraphSAGE-LSTM architecture with 256 hidden channels. The bidirectional design processes graph information in both forward and reverse edge directions using separate SAGEConv layers, concatenating the resulting embeddings (512 dimensions) before applying ReLU activation and 20\% dropout. Layer-wise embeddings are sequentially fed into an LSTM network to capture temporal dependencies across graph convolution layers. The final MLP classifier includes residual connections and dropout regularization for robust prediction. The complete pipeline framework is illustrated in Figure~\ref{fig:graphSAGE_framework}. 

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/graphSAGE_framework.png}
    \caption{Bidirectional GraphSAGE-LSTM architecture framework for Method E showing the flow from input features through bidirectional GraphSAGE layers, LSTM processing, and final classification.\cite{10851921}}
    \label{fig:graphSAGE_framework}
\end{figure}
...
\subsubsection{Official Competition Performance}
Method A (Dual-Branch) performed exceptionally well, successfully identifying \textbf{all 10 clean circuits} (100\% Precision) while only misclassifying 1 of the 20 Trojaned circuits. This yields a final F1-Score of 97.4\% (Table~\ref{tab:competition_results_B}).

\begin{table}[h!]
    \centering
    \caption{Method B: Final Circuit-Level Performance}
    \label{tab:competition_results_B}
    \begin{tabular}{lc}
        \toprule
        \textbf{Metric} & \textbf{Score} \\ \midrule
        Accuracy      & 96.7\% \\
        Precision     & 100.0\% \\
        Recall (TPR)  & 95.0\% \\
        \textbf{F1-Score}      & \textbf{97.4\%} \\ \bottomrule
    \end{tabular}
\end{table}
\subsubsection{Gate-Level Detection Consistency}
Gate-level analysis showed a high median F1-score of 0.9787 across the 20 Trojan cases, indicating reliable localization. The final model achieved significant gains over its pre-tuning variant on challenging cases.
...
% --- RESULTS FOR METHOD E (NEW) ---
\subsection{Performance of Method E}
\subsubsection{Official Competition Performance}
Method E (Bidirectional GraphSAGE-LSTM w/ Augmentation) 
achieved robust circuit-level classification with 83.33\% accuracy (25 out of 30 circuits correctly classified)
and 90.00\% Recall. 

\begin{table}[h!]
    \centering
    \caption{Method E: Final Circuit-Level Performance}
    \label{tab:competition_results_E}
    \begin{tabular}{lc}
        \toprule
        \textbf{Metric} & \textbf{Score} \\ \midrule
        Accuracy & 83.33\% \\
        Precision & 85.71\% \\
        Recall (TPR) & 90.00\% \\
        \textbf{F1-Score} & \textbf{87.80\%} \\\bottomrule
    \end{tabular}
\end{table}

\subsubsection{Gate-Level Detection Consistency}
Gate-level analysis showed a median F1-score of 0.62 across the 20 Trojan cases. 
Among cases where detection was attempted, the method achieved varying performance with the best case reaching an F1-score of 0.92. The bidirectional GraphSAGE-LSTM architecture demonstrated particular effectiveness on certain Trojan patterns, while facing challenges with detection sensitivity in other cases, highlighting areas for future model refinement.

% The bidirectional GraphSAGE-LSTM architecture with data augmentation strategy successfully captured both forward and backward graph dependencies for improved model generalization across diverse circuit topologies. The weighted Cross-Entropy loss with class weights [1.0, 50.0] effectively addressed severe class imbalance, while the LSTM component captured temporal dependencies across graph convolution layers, demonstrating competitive performance in hardware Trojan detection.
