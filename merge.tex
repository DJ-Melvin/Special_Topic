...
% ===== NEW ABSTRACT =====
\begin{abstract}
Recent advancements in Machine Learning (ML), particularly Graph Neural Networks (GNNs), have shown significant potential for Hardware Trojan (HT) detection by identifying anomalous patterns in gate-level netlists. This approach eliminates the dependency on a "golden chip" and promises high accuracy. However, challenges such as severe class imbalance and the stealthy nature of Trojans remain. In this paper, we explore and compare three distinct GNN-based frameworks to address these challenges for the 2025 CAD Contest. The first method (Method A) employs a dual-model GraphSAGE system to perform classification at both circuit and gate levels. The second method (Method B) utilizes a dual-branch architecture, fusing node-level BiGCN features with subgraph-level MLP features. The third method (Method C) integrates a BiGCN with a custom Threshold-Aware Focal Loss to combat class imbalance. We present the architecture, feature engineering, and post-processing strategies for each method, culminating in a comparative analysis of their effectiveness.
\end{abstract}
...
We developed and explored three distinct GNN-based frameworks for hardware Trojan detection. Each method is detailed below.
...
This section details the dataset generation, feature engineering, and training procedures for each of the three methodologies.
...
(Please fill in the final Accuracy, Precision, Recall, and F1-Score for Method A here. You can use a table similar to the one in Method C.)
...
Here, we compare the three proposed methods (A, B, and C) across several key dimensions based on their respective performance.
...
\begin{table}[h!]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        A: Dual-GraphSAGE & 0.9391 & 0.5386 & 0.952 & 0.688 \\
        B: Dual-Branch GNN & (Fill in) & (Fill in) & (Fill in) & (Fill in) \\
        C: BiGCN w/ Loss & 90.0\% & 87.0\% & 100.0\% & 93.0\% \\
        \bottomrule
    \end{tabular}
\end{table}

\paragraph{Analysis of Feature Engineering}
(Fill in your analysis here. Compare the effectiveness of the 17-dim (Method A), 41-dim (Method B), and 18-dim (Method C) feature sets. Did the inclusion of simulation statistics (Method B) provide a noticeable advantage over the purely structural features of Methods A and C?)

\paragraph{Analysis of Model Architecture}
(Fill in your analysis here. Discuss the trade-offs between the three architectures. Was the dual-model system of Method A (circuit-level + gate-level) more effective at separating tasks? How did the dual-branch fusion (Method B) compare to the dense concatenation of intermediate layers (Method C)?)

\paragraph{Analysis of Post-Processing}
(Fill in your analysis here. Compare the three distinct post-processing strategies. Method A used neighbor voting and a threshold of 15. Method B used a 3-step filter (consistency, small-group pruning, threshold 10). Method C used a simple count threshold of 20. Which provided the best balance between reducing false positives (Precision) and not harming true positives (Recall)?)

\paragraph{Analysis of Imbalance Handling}
(Fill in your' analysis here. Method C explicitly used a `Threshold-Aware Focal Loss`, while Method B used a standard `Focal Loss`. Based on the results, how critical was this custom loss function in achieving high recall?)
...
This work not only presents three viable solutions to the HT detection problem but also provides a comparative analysis that offers valuable insights into the strengths and weaknesses of different design choices. Future work could involve creating hybrid models that combine the most effective components from each of these three approaches, or applying this methodology to a wider range of public benchmarks.
